    ...
    def run(self, epochs=1):
        ...
        for i in range(1, epochs + 1):
            ...
            save_path = os.path.join(
                self.config.weight_save_dir, "epoch_{}.pkl".format(i)
            )
            ...
            if acc >= self.pre_acc:
                ...
                self.best_epoch = i
                state = {"model_state": self.model.state_dict()}
                torch.save(state, self.config.weight_loader_dir)
...
class singleInputTrainer(BaseTrainer):
    def __init__(
        self,
        model=None,
        optimizer=None,
    ...
    def train(self, epoch):
        print(
            "############# training {} #############".format(
                len(self.train_dataloader) * self.config.batch_size
            )
        )
