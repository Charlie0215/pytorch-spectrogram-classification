...
class urbanSoundLoader(data.Dataset):
    def __init__(
        self, test_folder="1", is_train=True, spec_path="./urbansound_spectrogram"
    ):
        """
        ...
        if self.is_train:
            ...
            self.aug_opt_1 = transforms.Compose(
                [
                    ...
                    AddGaussianNoise(mean=0.0, std=1.0, p=0.5),
                    transforms.ToTensor(),
                ]
            )
    ...
    def __getitem__(self, index):
        ...
        # use spectrogram, chromagram, mfcc
            ...
            if self.is_train:
                ...
                features = [self.toTensor(x) for x in features]
            if not self.is_multihead:
                img = torch.cat((features[0], features[1], features[2]), dim=0)
            else:
                img = features
        else:
            raise ValueError(
                "{} is not a valid option for input data".format(self.input_type)
            )
