...
def prepare_urbansound(
    ...
    for row in tqdm(valid_data.itertuples()):
        ...
        if ps.shape != (128, 128):
            continue
        img_path = (
            row.path.split(".wav")[0].split("/")[1]
            + "_{}".format(row.classID)
            + "_{}".format(row.fold)
            + ".png"
        )
        print(img_path)
    ...
    for folder in os.listdir(audio_root):
        ...
        for file in os.listdir(folder_path):
            ...
            spectrogram = librosa.feature.melspectrogram(
                y=y1, n_mels=128, hop_length=512
            )
            ...
            chroma_stft = librosa.feature.chroma_stft(
                y=y1, sr=sr1, n_chroma=12, n_fft=2048
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv", type=str, default="../data/UrbanSound8K/metadata/UrbanSound8K.csv"
    )
